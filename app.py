...
def load_dataset(path: Path) -> pd.DataFrame:
    ...
    required = {
        ...
        "kelompok_rentan",
        "sumber",
        "saran_konsumsi_sehat",
        "konsumsi_tidak_sehat",
    }
    if not required.issubset(df.columns):
        st.error("❌ Kolom dataset belum sesuai skema.")
        st.stop()
...
def detect_objects(path: str, model: YOLO, conf: float = 0.25):
    res = model.predict(source=path, device="cpu", save=False, conf=conf, verbose=False)
    foods: List[str] = []
    for r in res:
...
def extract_ingredients(txt: str, df: pd.DataFrame) -> Set[str]:
    ...
    return {
        nm
        for nm in df["nama_lower"].unique()
...
if "context_set" not in st.session_state:
    st.session_state.context_set: set[str] = set()
if "context_df" not in st.session_state:
    st.session_state.context_df = pd.DataFrame()
...
if st.session_state.ready:
    ...
    if question:
        ...
        new_ing = (
            extract_ingredients(question, master_df) - st.session_state.context_set
        )
        if new_ing:
